...
class TextureDrivenWatermark(nn.Module):
    ...
    def __init__(self, use_attention=True, fixed_step=10):
        super(TextureDrivenWatermark, self).__init__()
        self.use_attention = use_attention
        self.fixed_step = fixed_step  # 固定步长，仅在不使用注意力时使用
    ...
    def compute_lbp(self, rgb_tensor):
        ...
        for i in range(batch_size):
            # 转换为灰度和numpy
            gray = (
                0.299 * rgb_tensor[i, 0]
                + 0.587 * rgb_tensor[i, 1]
                + 0.114 * rgb_tensor[i, 2]
            )
            ...
            # 转换回张量
            lbp_tensor = (
                torch.from_numpy(lbp).float().unsqueeze(0).to(rgb_tensor.device)
            )
    ...
    def embed_watermark(self, frame, watermark, texture_mask=None):
        ...
        # 与原始UV通道组合
        watermarked_yuv = torch.cat(
            [watermarked_y.unsqueeze(1), weighted_yuv[:, 1:2], weighted_yuv[:, 2:3]],
            dim=1,
        )
        ...
        # 确保值在有效范围内
        watermarked_rgb = torch.clamp(watermarked_rgb, 0.0, 1.0)
    ...
    def extract_watermark(
        ...
        if apply_correction and reference_frame is not None:
            # 将张量转换为numpy以进行SIFT
            watermarked_np = (
                watermarked_frame.squeeze(0).permute(1, 2, 0).detach().cpu().numpy()
            )
            reference_np = (
                reference_frame.squeeze(0).permute(1, 2, 0).detach().cpu().numpy()
            )
            ...
            # 转换回张量
            corrected_tensor = (
                torch.from_numpy(corrected_gray)
                .float()
                .unsqueeze(0)
                .unsqueeze(0)
                .to(watermarked_frame.device)
            )
    ...
    def forward(self, frame, watermark):
        ...
        # 确保值在有效范围内
        watermarked_frame = torch.clamp(watermarked_frame, 0.0, 1.0)
